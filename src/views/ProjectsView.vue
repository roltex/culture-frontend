  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div v-else class="text-center py-12">
        ...
        <h3 class="mt-2 text-sm font-medium text-gray-900">{{ t('message.noProjects') }}</h3>
        <p class="mt-1 text-sm text-gray-500">{{ t('projects.noProjectsDescription') }}</p>
      </div>
    </div>
  </div>
</template>
...
const { data, isLoading, isError, error, refetch } = useQuery({
  ...
  queryFn: async () => {
    const response = await api.get(`/projects?page=${currentPage.value}`)
...
const goToPage = (page: number) => {
  ...
  if (page === 1) {
    ...
    router.push({ name: 'ProjectsPage', params: { page: page.toString() } })
  }
}
