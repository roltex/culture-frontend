<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <!-- Content -->
    <div v-else-if="legalAct" class="min-h-screen bg-gray-50">
      <!-- Header Section -->
            ...
            <div class="flex items-center justify-between">
              <div>
                ...
                <div class="flex items-center space-x-4 text-sm text-gray-500">
                  <span v-if="legalAct.published_at">{{ formattedPublishedDate }}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-12 lg:gap-6">
          ...
          <article class="lg:col-span-8 bg-white rounded-lg shadow-sm overflow-hidden p-6 md:p-8">
            ...
            <AttachmentViewer :attachments="legalActAttachments" />
          </article>

          <!-- Sidebar -->
          <aside class="lg:col-span-4 mt-10 lg:mt-0 lg:sticky top-24 self-start">
            <div class="bg-white rounded-lg shadow-sm">
              <h3 class="text-xl font-semibold text-gray-900 px-6 pt-6 pb-4">{{ t('legalActs.details') }}</h3>
              <div class="px-6 pb-6 space-y-4">
                <!-- Published Date -->
                <div v-if="legalAct.published_at">
                ...
                <!-- Attachments Count -->
                <div v-if="legalAct.attachments && legalAct.attachments.length > 0">
                  <span class="text-sm font-medium text-gray-500 block mb-1">
                    {{ t('legalActs.attachments') }}
                  </span>
                  <span class="text-lg font-semibold text-gray-900">
                    {{ legalAct.attachments.length }}
                  </span>
                </div>
              </div>
            </div>
...
const { getLegalAct } = useLegalActs()
...
const formattedPublishedDate = computed(() => {
  if (!legalAct.value?.published_at) return ''
  return formatEventDateLocalized(legalAct.value.published_at, locale.value, 'short')
})

// Get localized content for the main content area
const localizedContent = computed(() => {
  if (!legalAct.value?.description) return ''
  return getLocalizedContent(legalAct.value.description, locale.value)
})
...
// Map legal act attachments to the expected format for AttachmentViewer
const legalActAttachments = computed(() =>
  legalAct.value?.attachments?.map(att => ({
    name: getFileName(att.file_path),
    url: att.url,
    size: 0 // Default size since legal act attachments do not have a size
  })) || []
)
</script> 