  ...
  <div class="bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 py-12 sm:py-16 overflow-hidden">
    ...
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
      <div class="text-center mb-16">
        <h2 class="text-3xl font-bold tracking-tight text-slate-800 sm:text-4xl">
          {{ $t('home.other_structures.carousel_title') }}
        </h2>
    ...
    <div class="relative">
      ...
      <div v-else-if="isError" class="text-center">
        <p class="text-red-500">Could not load other structures.</p>
      </div>
      <swiper
        v-else-if="otherStructures && otherStructures.length"
      ...
      >
        <swiper-slide
          v-for="structure in otherStructures"
          :key="structure.id"
          class="!w-[200px] md:!w-[220px] group"
        >
          <a
            :href="structure.link || undefined"
            target="_blank"
          ...
          >
            <div class="flex-grow flex items-center justify-center h-20">
              <img
                ...
                :src="getImageUrl(structure.image)"
                :alt="getLocalizedContent(structure.title, locale)"
                @error="handleImageError"
              />
              <!-- Fallback SVG icon for structures without image -->
              <div v-if="!structure.image" class="w-16 h-16 bg-slate-200 rounded-lg flex items-center justify-center">
            ...
            <p class="mt-4 text-sm font-semibold text-slate-700 transition-colors group-hover:text-primary flex-shrink-0">
              {{ getLocalizedContent(structure.title, locale) }}
            </p>
          </a>
        </swiper-slide>
      </swiper>
...
import { useOtherStructures, getLocalizedContent } from '@/composables/useOtherStructures'
...
const { getOtherStructures } = useOtherStructures()
const { data: otherStructures, isLoading, isError } = getOtherStructures()
</script>
